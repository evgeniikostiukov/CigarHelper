...
- **2026-04-02** — Коллекция: поле `UserCigar.Quantity`, миграция `AddUserCigarQuantity`; create/update DTO и ответы с `quantity`; `POST /api/cigars/{id}/smoked` списывает одну штуку или финализирует выкуривание; дашборд — сумма остатков в хьюмидоре и по брендам; `CigarForm.vue` — количество, кнопка «прокурена», CTA в пустом поиске автокомплита для добавления без модерации; `CigarList` — уточнён текст подтверждения; `openapi.json` + `generate:api`.
- **2026-04-02** — UI остатка: `CigarList.vue` — бейдж «N шт.» на превью и строка «Остаток» в карточке (для невыкуриенных), `memoKey` учитывает `quantity`/`isSmoked`; `CigarDetail.vue` — поле «Остаток» в основной информации.

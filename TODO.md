...
- [x] **UX:** клик по пользователю — переход на публичный профиль `PublicUserProfile` (`/u/:username`): `ReviewList` / `ReviewDetail` (аватар + имя), `CigarCommentsPanel`, `AdminCigarComments`; только при `isAuthorProfilePublic` в DTO; иначе текст + tooltip. Префетч: `GET /api/public/users/{username}/visibility` + кэш, вызов с `@mouseenter` на ссылке. Общий компонент `PublicProfileAuthorBlock.vue`, хелпер `publicProfileRoute.ts`.
  - [x] **Подзадача:** блок «Недавние обзоры» на дашборде — `RecentReviewDto`: `username`, `isAuthorProfilePublic`, `userAvatarUrl`; UI: `PublicProfileAuthorBlock` (префетч видимости как в списке обзоров). **Бэклог:** при появлении ленты/рекомендаций с чужими авторами — те же поля в DTO и тот же компонент.
- [x] **Обзоры:** удаление для автора — мягкое (`Review.DeletedAt`), не показывать в списках и публичных местах; восстановление — `Admin`/`Moderator`: `GET /api/admin/reviews/deleted`, `POST /api/admin/reviews/{id}/restore`; миграция `ReviewSoftDeleteDeletedAt`; SPA: `AdminDeletedReviews.vue`, `adminDeletedReviewsService`, маршрут `/admin/reviews-deleted`, навигация в `AdminLayout` / карточка на `AdminDashboard`.
- [ ] **Обзоры (отображение):** на странице со списком обзоров в превью/теле обзора видна «голая» HTML-разметка (теги как текст или лишняя оболочка документа) — исправить: единый формат вывода (санитизированный `v-html` / excerpt из TipTap JSON / plain text) в `ReviewList.vue` и при необходимости `ReviewDetail.vue`; не дублировать `<html>`/`<body>` в контенте.
- [ ] **Обзоры — комментарии:** возможность оставлять комментарии под обзором (`Review`): сущность + миграция, CRUD/list API, права (авторизованные / публичное чтение по решению), UI на `ReviewDetail.vue` (панель по аналогии с `CigarCommentsPanel` или общий компонент); при необходимости очередь модерации в админке (как у `CigarComment`); тесты API.
- [ ] **Обзоры — навигация:** на `ReviewDetail.vue` кнопка «Открыть сигару в коллекции» сейчас падает с ошибкой; нужен корректный переход к **базовой сигаре** (`CigarBase` / каталог, `Review.CigarBaseId`) — маршрут или диалог карточки; при необходимости переименовать кнопку, если цель — справочник, а не `UserCigar` в коллекции.
- [ ] **Справочник (`CigarBase`):** удаление записи каталога для ролей **Admin** и **Moderator** — эндпоинт (жёсткое или мягкое), политика при связанных `UserCigar`, `Review` и изображениях (блокировка / каскад / перенос); UI в базе сигар и/или диалоге карточки; интеграционные тесты.
- [ ] **Админ — комментарии:** к существующему блоку (`AdminCigarComments.vue`, очередь модерации) добавить фильтр **«Все комментарии»** и **«На проверке»** (только ожидающие модерации); при необходимости query-параметр в `AdminCigarCommentsController` / списке API и тесты.
- [ ] **Админ — пользователи:** на странице списка (`AdminUsers.vue`) явно выводить **общее количество пользователей** (шапка/подзаголовок), по данным `totalCount` из `GET /admin/users` (или отдельный aggregate, если нужно число «всего в системе» независимо от фильтров страницы).
...
- **2026-03-30** — E2E: `e2e/tests/smoke-journey.spec.ts` — регистрация через UI или вход по env учётных данных, хьюмидоры/форма, сигары, обзоры, `/cigar-bases`; описание в `e2e/README.md`. *(2026-04-08: `E2E_USERNAME` + `E2E_PASSWORD`.)*
...
- **2026-04-09** — `nginx.docker.conf`: `proxy_pass http://api:8080` без URI — запросы к `/api/*` доходят до API как `/api/*` (исправление 404 на `/api/auth/register` и остальные маршруты).
- **2026-04-09** — Docker prod example: в `environment` API добавлены `AllowedHosts` (дефолт `*.sslip.io` + localhost, переопределение `ALLOWED_HOSTS` в `.env`); `.env.example` и `docs/docker.md` — чеклист против 400 Invalid Hostname.
- **2026-04-09** — Прод в Docker: `docker-compose.production.example.yml` (Production, ForwardedHeaders + `TrustPrivateNetworks`, CORS из `PUBLIC_WEB_ORIGIN`), `.env.example` / `docs/docker.md`; nginx — `X-Forwarded-Proto`/`X-Forwarded-Host`; API — опция `ForwardedHeaders:TrustPrivateNetworks` и RFC1918 в `KnownIPNetworks`; `docker-compose.production.yml` в `.gitignore`.
- **2026-04-09** — Прод-конфиг API: `appsettings.Production.json` — `AllowedHosts` дополнен `*.sslip.io` (динамические имена sslip.io), в CORS добавлен `https://31-177-83-239.sslip.io`, чтобы убрать 400 Invalid Hostname при вызове `https://31-177-83-239.sslip.io/api/...` за прокси.
- **2026-04-09** — Docker: закоммичен шаблон `docker-compose.override.example.yml` (CORS для 127.0.0.1 и подсказка LAN); `docs/docker.md` — копирование в `docker-compose.override.yml`.
...
- **2026-04-10** — Справочник сигар и брендов: API — `POST`/`PUT`/`DELETE` `/api/Brands` и `POST`/`PUT` `/api/Cigars/bases` только роли **Admin**, **Moderator**; фронт — меню «База сигар»/«Бренды» для любого JWT, `/brands` без `requiresAdmin`, CRUD брендов и правка базы в UI только staff; `CigarBases` — главная кнопка для пользователя ведёт в `CigarNew`, для staff — новая запись справочника; `CigarDetailDialog` — `canEditCatalog`; интеграционные тесты в `ApiAuthorizationAndContractsIntegrationTests`; обновлены `docs/memory-bank/code-map.md` и `docs/memory-bank/frontend/code-map.md`.
...
- **2026-04-12** — Галерея при создании карточки справочника из `CigarForm`: `CreateCigarBaseFormRequest` — `ImageUrls` / `ImageUrlIsMain`; `CigarsController.AppendCigarBaseImagesFromUrlsAsync` + `IOptions<ImageUploadOptions>`; `POST /api/cigarimages/upload-by-url` без ограничения staff; диалог «Добавить в справочник» — файлы и URL (multi), `buildDraftCigarBaseFormData` шлёт `NewImages` и `ImageUrls` в одном multipart.
- **2026-04-15** — Клик на автора → публичный профиль: `GET /api/public/users/{username}/visibility`, `IsAuthorProfilePublic` в DTO обзоров и комментариев; `PublicProfileAuthorBlock.vue`, префетч в `profileService`, тесты видимости в `ApiAuthorizationAndContractsIntegrationTests`; `ReviewList` / `ReviewDetail` / `CigarCommentsPanel` / `AdminCigarComments`; `publicUserProfileLocation` в `Profile.vue`.
- **2026-04-15** — Дашборд «Недавние обзоры»: `RecentReviewDto` — `Username`, `IsAuthorProfilePublic`, `UserAvatarUrl`; `DashboardService` / `Dashboard.vue` + `PublicProfileAuthorBlock`; `dashboardService` / `api.generated.ts`; тесты `DashboardServiceTests`, `Dashboard.test.ts`.
- **2026-04-15** — Обзоры: мягкое удаление (`Review.DeletedAt`), фильтр в `ReviewService` / `DashboardService`; staff `AdminReviewsController` (`GET .../deleted`, `POST .../restore`); миграция `ReviewSoftDeleteDeletedAt`; тесты `ReviewServiceTests`, `DashboardServiceTests`, `ApiAuthorizationAndContractsIntegrationTests`; `docs/memory-bank/code-map.md`.

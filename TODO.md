...
- [x] **Обзоры:** удаление для автора — мягкое (`Review.DeletedAt`), не показывать в списках и публичных местах; восстановление — `Admin`/`Moderator`: `GET /api/admin/reviews/deleted`, `POST /api/admin/reviews/{id}/restore`; миграция `ReviewSoftDeleteDeletedAt`.

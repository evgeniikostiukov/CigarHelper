...
- [x] **Обзоры (отображение):** `reviewContentDisplay.ts` — превью списка: plain excerpt из `summary`; деталь: `sanitizeReviewBodyForDisplay` (TipTap HTML или плейнтекст → параграфы), `DOMParser` снимает оболочку документа, DOMPurify; тесты `reviewContentDisplay.test.ts`; `ReviewList.vue`, `ReviewDetail.vue`.
- [x] **Обзоры — комментарии:** возможность оставлять комментарии под обзором (`Review`): сущность + миграция, CRUD/list API, права (авторизованные / публичное чтение по решению), UI на `ReviewDetail.vue` (панель по аналогии с `CigarCommentsPanel` или общий компонент); при необходимости очередь модерации в админке (как у `CigarComment`); тесты API.
- [x] **Обзоры — навигация:** на `ReviewDetail.vue` кнопка «Открыть сигару в коллекции» сейчас падает с ошибкой; нужен корректный переход к **базовой сигаре** (`CigarBase` / каталог, `Review.CigarBaseId`) — маршрут или диалог карточки; при необходимости переименовать кнопку, если цель — справочник, а не `UserCigar` в коллекции.

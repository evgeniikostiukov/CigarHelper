...
| Фильтры | Сворачиваемый блок в духе **CigarBases**: `@vueuse/core` **`useLocalStorage('cigar-list-filters-expanded', false)`** — по умолчанию **свёрнут**; кнопка разворота, при свёрнутом блоке и активных условиях — бейдж «Фильтры активны». **Клиентская** фильтрация полного списка: поиск (подстрока в `name` и `brand.name`, без учёта регистра), бренд (`Select` по `brand.id`), крепость и формат (`Select`, опции — уникальные непустые `strength` / `size` из загруженных сигар), хьюмидор (все / `humidorId != null` / `humidorId == null`), выкурена (все / `!isSmoked` / `isSmoked`). Сетка карточек и пустые состояния завязаны на **`displayedCigars`** (computed). Отдельный экран «Ничего не найдено» — если в коллекции есть сигары, но фильтр дал 0 строк. Сброс — кнопка в панели и на экране пустой выдачи |
| Карточка | **Карусель** (`Carousel`, PrimeVue, автоимпорт) вверху z-20, фикс. высота ~192px; клик по слайду/заглушке ведёт в деталь через `router.push`. Ниже — `router-link` на текстовый блок (размер/крепость/**количество** «N шт.», рейтинг, цена, «В хьюмидоре»). Футер: «выкурена» (`markCigarAsSmoked`), правка, удаление (`cigarService.deleteCigar`, confirm + toast) |
| Изображения | `GET /api/cigars` отдаёт объединённую галерею: фото UserCigar, затем фото CigarBase; миниатюры — `GET /api/cigarimages/{id}/thumbnail` (blob URL). Порядок слайдов: `orderUserCigarGalleryImages` в `cigarImageDisplay.ts`. Устаревший inline: `imageData` / base64 через `cigarImageInlineDataSrc`. **`watch` на `displayedCigars`** (deep) подгружает blob-миниатюры только для карточек, попавших в текущую выдачу |
...
| `data-testid` | `cigar-list`, `cigar-list-filters`, `cigar-list-filters-collapsed-hint`, `cigar-list-filters-toggle`, `cigar-list-search`, `cigar-list-filter-brand`, `cigar-list-filter-strength`, `cigar-list-filter-size`, `cigar-list-filter-humidor`, `cigar-list-filter-smoked`, `cigar-list-filter-actions`, `cigar-list-filter-reset`, `cigar-list-loading`, `cigar-list-skeleton`, `cigar-list-error`, `cigar-list-retry`, `cigar-list-filter-empty`, `cigar-list-filter-empty-reset`, `cigar-list-empty`, `cigar-list-empty-add`, `cigar-list-add`, `cigar-list-grid`, `cigar-card-{id}`, `cigar-card-quantity-{id}`, `cigar-smoke-{id}`, `cigar-edit-{id}`, `cigar-delete-{id}` |
| Scoped классы | `cigar-list-root`, `cigar-list-grain`, `cigar-card-enter`, `cl-list-filters-panel` (enter/leave + `prefers-reduced-motion`), `:deep` для мин. размера стрелок карусели |

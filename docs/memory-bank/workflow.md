...
Публикация готовых образов **api** и **web** в **GHCR** и деплой без сборки на сервере — раздел **«Образы в GHCR»** в том же [`docs/docker.md`](../docker.md) (workflow `publish-ghcr.yml`, файл `docker-compose.ghcr.example.yml`, переменные `GHCR_IMAGE_NAMESPACE` / `IMAGE_TAG` в `.env.example`, скрипт **`scripts/deploy-docker-ghcr.sh`**).

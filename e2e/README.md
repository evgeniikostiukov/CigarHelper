...
- **`tests/smoke-journey.spec.ts`** — регистрация через UI (уникальный email/username) **или** вход по переменным **`E2E_EMAIL`** и **`E2E_PASSWORD`** (обе заданы — регистрация не выполняется). Далее: хьюмидоры → форма создания → назад, «Мои сигары», «Обзоры», прямой переход на `/cigar-bases` (сетка карточек / пусто / ошибка API).

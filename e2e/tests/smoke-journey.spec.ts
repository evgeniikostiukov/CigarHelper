 ...
 * - По умолчанию: регистрация нового пользователя через UI (уникальный логин).
 * - Опционально: `E2E_USERNAME` + `E2E_PASSWORD` — только вход (без регистрации).
 */
test.describe('Smoke journey', () => {
  test('регистрация или вход, хьюмидоры, форма, сигары, обзоры, база сигар', async ({
    ...
    const envUsername = process.env.E2E_USERNAME?.trim();
    const envPassword = process.env.E2E_PASSWORD?.trim();
    const suffix = `${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const username = envUsername || `e2e_u_${suffix}`;
